...
from pepytools.util import get_polarization_matrix, center_of_mass
...
class TestUtil(unittest.TestCase):
    ...
    def test_center_of_mass(self):
        ...
        m = numpy.asarray([1.0])
        self.assertEqual(norm(center_of_mass(c, m)), 0.0)

        c = numpy.asarray([[0.0, 0.0, 0.0], [2.0, 0.0, 0.0]])
        m = numpy.asarray([0.5, 0.5])
        self.assertEqual(norm(center_of_mass(c, m)), 1.0)

    def test_get_pol_matrix(self):
        """ polarization matrix """
        p = Potential.from_file('tests/nomul.pot')
        pol = p.polarizabilities[0]
        mat = get_polarization_matrix(p)
